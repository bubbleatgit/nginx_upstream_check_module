...
ngx_http_upstream_check_status_post_handler(ngx_http_request_t *r)
{
    ...
    if(data.len>0)
    {
        ...
        ngx_url_decode(service_value.data, service_value.len, 
                       decode_service_value, sizeof(decode_service_value));
...
ngx_http_get_post_args_param(ngx_str_t *args, const char* name, ngx_str_t *value)
{
    ...
    {
        if(start<0 && i<(int)args->len-len && args->data[i]==name[0] && 
           !memcmp(&args->data[i],name,len))
        {
            start = i+len;
            i+=len;
        }
